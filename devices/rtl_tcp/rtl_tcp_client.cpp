...
//
#define	DEFAULT_FREQUENCY	(Khz (92000))
...
void	rtl_tcp_client::setVFOFrequency	(quint64 newFrequency) {
	if (!connected)
	   return;
	vfoFrequency	= (int32_t) newFrequency;
//	here the command to set the frequency
	sendVFO (newFrequency);
}

quint64	rtl_tcp_client::getVFOFrequency() {
	return  (quint64)vfoFrequency;
}
...
int cnt	= 0;
	...
	if (_I_Buffer -> GetRingBufferReadAvailable () > outputRate / 8) {
	   dataAvailable (_I_Buffer -> GetRingBufferReadAvailable ());
	}
}
