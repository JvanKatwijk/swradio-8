...
uint32_t samplerate_count;
	...
	linearityDisplay	-> display  (10);
	sensitivitySlider	-> setValue (10);
	sensitivityDisplay	-> display (10);
	vgaGain			= 5;
...
//
	...
	for (i = 0; i <  (int)samplerate_count; i ++) {
	   fprintf (stderr, "%d \n", myBuffer [i]);
	   if (abs (myBuffer [i] - 2112000.0) < distance) {
	      distance	= abs (myBuffer [i] - 2112000.0);
...
//	(selectedRate / 1000) vs (2112000 / 1000)
	convBufferSize		= selectedRate / 1000;
	for (i = 0; i < 2112; i ++) {
	   float inVal	= float (selectedRate / 1000);
	   mapTable_int [i] =  int (floor (i * (inVal / 2112.0)));
	   mapTable_float [i] = i * (inVal / 2112.0) - mapTable_int [i];
	}
	convIndex		= 0;
...
//
//	We only signal once every 50 msec
	      theBuffer	-> putDataIntoBuffer (temp, 2112);
	      if (++sampleCnt > 50) {
