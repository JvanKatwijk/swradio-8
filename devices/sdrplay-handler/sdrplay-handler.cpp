...
//	wprintf (L"API registry entry: %s\n", APIkeyValue);
	...
	gainSlider      -> setValue (
                    sdrplaySettings -> value ("externalGain", 40). toInt ());
        ppmControl      -> setValue (
...
//	and be prepared for changes in the settings
	...
	fprintf (stderr, "hardware version = %o\n", text);
...
static inline
int16_t	bankFor_rsp_1 (int32_t freq) {
	if (freq < 12 * MHz (1))
	   return 1;
	if (freq < 60 * MHz (1))
	   return 2;
...
static inline
int16_t	bankFor_rsp_2 (int32_t freq) {
	if (freq < 12 * MHz (1))
	   return 1;
	if (freq < 60 * MHz (1))
	   return 2;
