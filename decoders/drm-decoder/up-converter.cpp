...
#include	"up-converter.h"
	...
	drmConverter::drmConverter (int32_t inRate, int32_t outRate,
	                          int inSize) {
	this	-> inRate	= inRate;
	...
	drmConverter::~drmConverter	() {
	if (workerHandle != nullptr)
	   delete workerHandle;
}

bool	drmConverter::convert (std::complex<float> v,
	                       std::complex<float> *out, int *amount) {
	return workerHandle -> convert (v, out, amount);
}

int	drmConverter::getOutputSize	() {
