...
//
static inline
DSPCOMPLEX	valueFor (float amp, int16_t phase) {
...
DSPCOMPLEX getTimeRef (uint8_t Mode, int16_t symbol, int16_t carrier) {
struct timeCells *table = tableforMode (Mode);
int16_t i;
...
DSPCOMPLEX getFreqRef (uint8_t Mode, int16_t symbol, int16_t carrier) {
struct timeCells *table;
int16_t	i;
...
DSPCOMPLEX	getPilotValue (uint8_t Mode,
...
float timeCorrelate (DSPCOMPLEX *vec, uint8_t Mode, uint8_t Spectrum) {
struct timeCells *ref = tableforMode (Mode);
int16_t	i;
DSPCOMPLEX	sum	= 0;

	for (i = 0; ref [i]. index != -1; i ++)
	   sum +=  vec [ref [i]. index - Kmin (Mode, Spectrum)] *
	                   conj (valueFor (sqrt (2), ref [i]. phase));
	return real (sum * conj (sum));
}
//
