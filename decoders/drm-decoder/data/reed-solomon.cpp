	...
	for (j = rootCount - 1; j >= 0; j--) {
	   ...
	   for (i = omega_degree; i >= 0; i--) {
	      if (omega [i] != codeLength) {
	         uint16_t xx1 = myGalois. multiply_power (omega [i],
 ...
 */
	   for (i = min (lambda_degree, nroots - 1) & ~1;
	      ...
	      if (Lambda [i + 1] != codeLength) {
	         uint16_t xx6 = myGalois. pow_power (i, rootTable [j]);
	         uint16_t xx7 = myGalois. multiply_power (Lambda [i + 1],
	                                                  xx6);
	         den	= myGalois. add_poly (den, myGalois. power2poly (xx7));
	      }
	   }
...
/*	Apply error to data */
	   if (num1 != 0) {
	      if (locTable [j] >=  uint8_t (codeLength - nroots))
	         ...
	         uint16_t yy1	= myGalois. poly2power (num1);
	         uint16_t yy2	= myGalois. poly2power (num2);
	         uint16_t yy3	= codeLength - myGalois. poly2power (den);
	         uint16_t yy4	= myGalois. multiply_power (yy1, yy2);
	         yy4		= myGalois. multiply_power (yy4, yy3);
	         uint16_t corr	= myGalois. power2poly (yy4);
	         data [locTable [j]] =
